...
const AdminChat = () => {
  ...
  const sendMessage = () => {
    if (inputMessage.trim() === '' || !socket || !selectedChat || !isConnected) {
      if (!isConnected) {
  ...
  const clearAllChats = async () => {
    ...
    try {
      const response = await axios.delete(`${BACKEND_URL}/api/chat/admin/clear-all`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
              ...
              >
                <div className="conversation-header">
                  <span className="conversation-user-name">
                    {conv.userName || `User ${conv.conversationId.slice(5, 13)}...`}
                  </span>
          ...
          <div className="chat-container">
            <div className="chat-header">
              <h3 className="chat-title">
                Chat dengan {selectedConversation?.userName || `User ${selectedChat.slice(5, 13)}...`}
              </h3>
                ...
                messages.map((msg, index) => (
                  ...
                  >
                    <div className="chat-bubble">
                      <div className="chat-sender">{getSenderDisplayName(msg)}</div>
