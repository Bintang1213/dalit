...
const editFood = async (req, res) => {
  try {
    const { id, name, description, price, category, status, isRecommended } =
      req.body; // ✅ Tambahkan isRecommended
    ...
    food.isRecommended =
      isRecommended !== undefined ? isRecommended : food.isRecommended; // ✅ Update isRecommended
...
const updateStatus = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Server error saat update status" });
  }
};
...
const updateRecommendationStatus = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "Server error saat update rekomendasi",
      });
  }
};
